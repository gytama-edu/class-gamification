...
export class SupabaseClassroomRepository implements ClassroomRepository {
  ...
  async getClasses(): Promise<Classroom[]> {
    ...
    const { data, error } = await supabase.from('classes').select('*').eq('is_archived', false).order('created_at', { ascending: false });
  ...
  async getClassroomDashboard(classId: string): Promise<ClassroomDashboardData> {
    ...
    if (activeMeeting) {
      const { data: studentsData, error: studentsError } = await supabase
        ...
        .eq('class_id', classId)
        .eq('is_active', true)
        .eq('student_meeting_states.meeting_id', activeMeeting.id);
      
      if (studentsError) throw studentsError;

      studentsWithStates = studentsData.map((s: any) => ({
        ...s,
        lives_remaining: s.student_meeting_states?.[0]?.lives_remaining ?? classroom.max_lives,
      }));
    } else {
      const { data: studentsData, error: studentsError } = await supabase
        .from('students')
  ...
  async createClass(input: { name: string; level_name: string; max_lives: number; }): Promise<Classroom> {
    ...
    const { data: newClass, error: fetchErr } = await supabase.from('classes').select('*').eq('id', data).single();
  ...
  async archiveClass(classId: string): Promise<void> {
    if (!supabase) throw new Error("Supabase not initialized");
    const { error } = await supabase.rpc('archive_class', { p_class_id: classId });
    if (error) throw error;
  }

  async getStudents(classId: string): Promise<DbStudent[]> {
    if (!supabase) throw new Error("Supabase not initialized");
    const { data, error } = await supabase.from('students').select('*').eq('class_id', classId).order('created_at');
    if (error) throw error;
    return data || [];
  }
  ...
  async addStudent(classId: string, input: { display_name: string; }): Promise<DbStudent> {
    ...
    const { data: student, error: fetchErr } = await supabase.from('students').select('*').eq('id', data).single();
    if (fetchErr) throw fetchErr;
    return student;
  }
  ...
  async getActiveMeeting(classId: string): Promise<Meeting | null> {
    if (!supabase) throw new Error("Supabase not initialized");
    const { data, error } = await supabase.from('meetings').select('*').eq('class_id', classId).eq('status', 'active').maybeSingle();
    if (error) throw error;
    return data;
  }
  ...
  async getStudentProfile(studentId: string): Promise<StudentWithCurrentState | null> {
    ...
    const { data: meeting } = await supabase.from('meetings').select('*').eq('class_id', student.class_id).eq('status', 'active').maybeSingle();
    
    let lives_remaining = 0;
    if (meeting) {
      const { data: state } = await supabase.from('student_meeting_states').select('lives_remaining').eq('student_id', studentId).eq('meeting_id', meeting.id).maybeSingle();
