...
export const MyClasses: React.FC = () => {
  const { classes, isLoadingClasses, archiveClass, restoreDefaultData } =
    useAppContext();
      ...
      {classes.length === 0 ? (
        <div className="bg-cosmic-panel border border-slate-800 rounded-3xl p-12 text-center">
          ...
          <p className="text-slate-400 max-w-md mx-auto mb-8">
            You don't have any active classes yet. Create your first class to
            start gamifying your lessons.
          </p>
          <button
            onClick={() => navigate("/teacher/classes/new")}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {classes.map((cls) => (
            <div
              key={cls.id}
              className="bg-cosmic-panel border border-slate-800 rounded-3xl p-6 hover:border-cosmic-cyan/30 transition-all group flex flex-col h-full"
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="relative group/menu">
                  ...
                  <div className="absolute right-0 top-full mt-1 w-32 bg-slate-800 border border-slate-700 rounded-xl shadow-xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-10 overflow-hidden">
                    <button
                      onClick={() =>
                        navigate(`/teacher/classes/${cls.id}/settings`)
                      }
