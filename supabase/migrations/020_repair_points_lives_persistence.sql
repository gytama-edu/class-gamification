-- Migration 020: Repair Points and Lives Persistence
...
BEGIN
  ...
  IF v_lives_remaining IS NULL THEN
    ...
    RETURNING lives_remaining INTO v_lives_remaining;
    -- Already at max, so no event if we just repaired it to max?
    -- But user asked to restore life. Let's just set to max.
  ELSE
